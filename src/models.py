...
class UNetGenerator(nn.Module):
    """
    Original pix2pix U-Net generator adapted for single-channel input (grayscale)
    and 2-channel output (ab).
    """
...
class PatchGANDiscriminator(nn.Module):
    ...
    Concatenates input [L] and target [ab or L+ab or RGB] -> real/fake patch.
    """
    ...
    def forward(self, input_image, target_image):
        """
        Args:
            input_image (Tensor): shape (bs, in_channels, H, W)
            target_image (Tensor): shape (bs, target_channels, H, W)
        """
